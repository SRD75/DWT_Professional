	...
	<?php if ( have_comments() ) : ?>
		<h2 class="comments-title">
			...
			printf( _nx( 'One thought on &ldquo;%2$s&rdquo;', '%1$s thoughts on &ldquo;%2$s&rdquo;',
				get_comments_number(), 'comments title', 'dwt_professional' ),
		...
		<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // are there comments to navigate through. ?>
			<nav class="comment-navigation" id="comment-nav-above">
				<h1 class="screen-reader-text"><?php _e( 'Comment navigation', 'dwt_professional' ); ?></h1>
				<?php if ( get_previous_comments_link() ) { ?>
					<div class="nav-previous"><?php previous_comments_link( __( '&larr; Older Comments',
					'dwt_professional' ) ); ?></div>
				<?php }
...
if ( get_next_comments_link() ) { ?>
	...
	if ( ! comments_open() && '0' != get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) :
		...
		<p class="no-comments"><?php _e( 'Comments are closed.', 'dwt_professional' ); ?></p>
